...
ssFolders.forEach(folder => {
  ...
  let path = filePath + '/' + folder
  var names = fs.readdirSync(path);
  names.forEach(name => {
    if (name.indexOf('1415') >= 0) {
      exec(getCommandLine() + " \'" + filePath + '/' + folder + '/' + name + "'");
      let question = 'Current PDF: ' + name + '\n' +
          'Please type the last number of the correct year\n';
      let answer = rl.question(question);
      newName = name.slice(0,-9).trim() + '201' + answer + '.PDF';
